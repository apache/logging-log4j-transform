...
public class LoggerConversionHandlerExample {
    ...
    public void testFatal(final ListAppender app) {
        ...
        assertLocationEquals(methodName, app);
    ...
    public void testCatchingThrowing(final ListAppender app) {
        ...
        assertLocationEquals(methodName, app, 2);
    ...
    public void testTraceEntry(final ListAppender app) {
        ...
        LogEvent event = assertLocationEquals(methodName, app);
        ...
        event = assertLocationEquals(methodName, app);
        assertThat(event.getMarker()).isEqualTo(AbstractLogger.ENTRY_MARKER);
    ...
    public void testTraceExit(final ListAppender app) {
        ...
        event = assertLocationEquals(methodName, app, 2);
        assertThat(event.getMarker()).isEqualTo(AbstractLogger.EXIT_MARKER);
        assertThat(event.getMessage()).isInstanceOf(ExitMessage.class);
    ...
    /**
     * Asserts log event has the right location intended, the log request should right before this call
     *
     * @param methodName method name of the calling function
     * @param app list appender
     * @return the log event extracted
     */
    private static LogEvent assertLocationEquals(final String methodName, final ListAppender app) {
    ...
    private static LogEvent assertLocationEquals(
        ...
        if (difference < 0) {
            assertThat(location.getLineNumber()).isEqualTo(getCallerLineNumber(true) - 1);
        } else {
            assertThat(location.getLineNumber()).isEqualTo(getCallerLineNumber(false) - difference);
        }
        app.clear();
        return event;
    }

    private static int getCallerLineNumber(boolean isNested) {
        return Thread.currentThread().getStackTrace()[3 + (isNested ? 1 : 0)].getLineNumber();
    }
}
